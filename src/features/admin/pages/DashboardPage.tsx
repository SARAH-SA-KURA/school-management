...
const DashboardPage: React.FC = () => {
  ...
  // Build donut segments for filière chart
  ...
  return (
    <div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          ...
          <div className="flex justify-center gap-8 mb-4">
            <div className="text-center bg-gray-50 rounded-lg px-6 py-3">
              <p className="text-2xl font-bold text-gray-900">{String(absentCount).padStart(2, '0')}</p>
          ...
          <div className="flex items-center justify-center h-48">
            <div className="relative">
              ...
              <div className="absolute -bottom-2 -right-4 bg-white rounded-lg shadow px-2 py-1 text-center">
                <p className="text-sm font-bold text-gray-900">{absentCount}</p>
                <p className="text-xs text-gray-500">Absent</p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          ...
          <div className="flex items-center justify-center h-48">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <p className="text-xl font-bold text-gray-900">{totalByFiliere}</p>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="text-left py-3 px-4 font-medium text-gray-500">Date Inscription</th>
            ...
            <tbody>
              {recentStagiaires.map((s: any) => (
                <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 text-gray-600">{s.date_inscription ? new Date(s.date_inscription).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }) : '-'}</td>
                  <td className="py-3 px-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                      s.status === 'actif' ? 'bg-green-100 text-green-700' :
                      s.status === 'abandon' ? 'bg-red-100 text-red-700' :
                      s.status === 'suspendu' ? 'bg-yellow-100 text-yellow-700' :
        ...
        <div className="mt-4">
          <Link to={`${basePath}/stagiaires`} className="text-sm text-primary-600 hover:text-primary-700 font-medium">
            Voir tout
          </Link>
        </div>
      </div>
